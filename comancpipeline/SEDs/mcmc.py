...
Version 1.03 [Jan 2022]
Stuart Harper

- Modified the code so it follows a proper import structure.
- Made settings an argument, no more "import config.py" (but we can keep the dictionary structure)
'''
...
import numpy as np
from comancpipeline.SEDs.tools import SED
from comancpipeline.SEDs import default_config
    ...
    for k, v in default_config.settings.items():
        ...
        if (k in settings) and isinstance(v,dict):
            for k1,v1 in v.items():
                if not k1 in settings[k]:
                    settings[k][k1] = v1
    return settings

def mcmc(nu, flux, flux_err, beam, excluded, settings):
