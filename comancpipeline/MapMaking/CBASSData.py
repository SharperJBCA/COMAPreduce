...
class CBASSData(object):
    ...
    def tod_iqu(self):
        ...
        tod[0,mask] -= np.nanmedian(tod[0,mask])
        return tod.flatten()
    ...
    def weights_iqu(self):
        ...
        special_weights,_ = self.special_weights_iqu
        weights[np.abs(special_weights) < 0.05] = 0
    ...
    def special_weights_iqu(self):
        ...
        return special_weights.flatten(),special_weights_rot.flatten()
...
class CBASSDataSim(CBASSData):
    ...
    def tod_iqu(self):
        tod = np.zeros((3,self.nsize))
        mask = (self.flag == 0)
        pixels = hp.ang2pix(1024,np.pi/2 - self.dec[:self.nsize],self.ra[:self.nsize]).astype(int)
        pixels = pixels[mask] 
        ...
        tod[1,mask] = Q*np.cos(2*self.pa[mask])  + np.random.normal(size=I.size,scale=1/np.sqrt(self.wI1[mask] + self.wI2[mask]))
        tod[2,mask] = U*np.sin(2*self.pa[mask])  + np.random.normal(size=I.size,scale=1/np.sqrt(self.wI1[mask] + self.wI2[mask]))
        return tod.flatten()
...
def  read_comap_data_iqu(_filelist, offset_length=100, ifile_start=0, ifile_end=None,nside=512): 
    ...
    all_cbass_data = [] 
    for i,filename in enumerate(filelist):
        cbass_data = CBASSData(filename,obsid=i+ifile_start,offset_length=offset_length,nside=nside)
        nend = nstart + cbass_data.nsize*3
        ...
        special_weights[nstart:nend],special_weights_rot[nstart:nend] = cbass_data.special_weights_iqu
    ...
    good_offsets = CBASSData.calc_empty_offsets(flags, offset_length=offset_length)
    return tod[good_offsets], weights[good_offsets], pointing[good_offsets], obsid[good_offsets], special_weights[good_offsets], special_weights_rot[good_offsets], all_cbass_data